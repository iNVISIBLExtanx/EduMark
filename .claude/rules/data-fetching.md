...
| `useBatchDetail` | `/api/batches/[id]` | single batch; returns `BatchDetail` with `paper_name: string \| null` and `subject_name: string \| null` (flattened from `question_papers.subjects.name` join) |
