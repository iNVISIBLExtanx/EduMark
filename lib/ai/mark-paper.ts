...
19. SELF-CHECK ARITHMETIC: Before finalising your JSON output, verify that for every question where you have populated a sub_questions array, the sum of all sub_questions[].awarded_marks equals the question's awarded_marks. If there is a discrepancy, correct the awarded_marks field to match the sub_questions sum (the sub-part breakdown is more precise than the top-level total). Also verify that no awarded_marks exceeds its corresponding max_marks.
</marking_rules>
